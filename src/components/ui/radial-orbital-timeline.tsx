...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('rounded-lg border bg-zinc-950 text-zinc-50 shadow-sm', className)}
      {...props}
    />
  )
);
...
export function RadialOrbitalTimeline({ timelineData }: RadialOrbitalTimelineProps) {
  ...
  const getStatusStyles = (status: TimelineItem['status']): string => {
    switch (status) {
      case 'completed':
        return 'text-zinc-950 bg-white border-white';
      case 'in-progress':
        return 'text-zinc-950 bg-yellow-400 border-yellow-400';
      case 'pending':
        return 'text-white bg-zinc-800 border-zinc-600';
      default:
        return 'text-white bg-zinc-800 border-zinc-600';
    }
  };
      ...
      <div className="relative w-full max-w-4xl h-[400px] sm:h-[600px] flex items-center justify-center">
        ...
        >
          ...
          <div className="absolute w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-[#facc15] via-[#eab308] to-[#ca8a04] anim-pulse flex items-center justify-center z-10">
            ...
            <div
              className="absolute w-20 h-20 sm:w-24 sm:h-24 rounded-full border border-white/10 anim-ping opacity-50"
              ...
              >
                ...
                <div
                  className={`
                    ...
                    ${
                      isExpanded
                        ? 'bg-white text-black border-white shadow-lg shadow-white/30 scale-125 sm:scale-150'
                        : isRelated
                          ? 'bg-white/50 text-black border-white anim-pulse'
                    ...
                    <CardContent className="text-xs text-white/80">
                      ...
                      <div className="mt-4 pt-3 border-t border-white/10">
                        ...
                        <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-yellow-400 to-[#ca8a04]"
