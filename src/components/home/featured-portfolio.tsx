...
const portfolioData = [
  ...
  },
  {
    ...
    description:
      'Portal berita olahraga berbasis link affiliate dengan performa tinggi & SEO optimized.',
          ...
          <div className="w-full md:w-1/3 md:sticky md:top-32 h-fit">
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mb-10">
                  {portfolioData[activeIndex].tech.map((t, i) => (
                    <span
                      key={i}
                      className="text-xs uppercase tracking-widest font-medium px-3 py-1.5 rounded-lg bg-slate-800/50 border border-slate-700 text-slate-300"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <Link href={portfolioData[activeIndex].link} target="_blank">
                  <Button
                    variant="outline"
                    className="gap-2 rounded-full border-white/20 hover:bg-white/5 active:scale-95 transition-all h-12 px-6"
                  >
            ...
            <div className="mt-8 md:hidden">
              <Link href="/portfolio" className="w-full">
                <Button
                  variant="outline"
                  className="w-full gap-2 rounded-full h-14 text-base border-white/20 hover:bg-white/5 active:scale-95 transition-all"
                >
            ...
            {portfolioData.map((project, index) => (
              ...
              >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block w-full overflow-hidden rounded-xl border border-slate-800 bg-[#0f172a] shadow-2xl relative group-hover:border-slate-700 transition-colors"
                >
                  {/* Browser Header */}
            ...
            <div className="hidden md:flex justify-center mt-10">
              <Link href="/portfolio">
                <Button
                  size="lg"
                  variant="outline"
                  className="gap-2 rounded-full border-white/20 hover:bg-white/5 h-16 px-10 active:scale-95 transition-all text-lg font-medium"
                >
