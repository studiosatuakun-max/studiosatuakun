    ...
    try {
      ...
      if (!fullText.trim()) {
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantId
              ? { ...m, content: 'AI tidak memberikan respons. Silakan coba lagi.' }
              : m
          )
        );
      }
      ...
      setMessages((prev) => {
        ...
        if (hasAssistant) {
          return prev.map((m) =>
            m.id === assistantId ? { ...m, content: `⚠️ Error: ${errorMsg}` } : m
          );
        }
        return [
          ...prev,
      ...
      <div className="container mx-auto px-4 sm:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <h2 className="text-3xl md:text-5xl font-bold mb-6 text-slate-900 dark:text-white">
            Konsultasikan Ide Anda dengan{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-600">
              AI Kami
            </span>
          </h2>
          <p className="text-lg text-slate-600 dark:text-slate-400">
            Ceritakan ide bisnis atau fitur aplikasi yang Anda inginkan. AI Tech Lead kami akan
            berdiskusi dengan Anda dan membuatkan draf prototipe antarmuka secara live di kanvas
            ini.
          </p>
        </div>
        ...
        <div
          className={`mx-auto bg-[#0c0c0c] border border-white/10 shadow-2xl rounded-2xl overflow-hidden transition-all duration-500 ease-in-out flex ${
            isExpanded
              ? 'w-full max-w-7xl h-[600px] flex-row'
              : 'w-full max-w-5xl h-[500px] flex-col'
          }`}
        >
          {/* Chat Area */}
          <div
            className={`flex flex-col h-full bg-[#0c0c0c] ${isExpanded ? 'w-1/3 min-w-[350px] border-r border-white/10' : 'w-full'}`}
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {messages.length === 0 && (
                <div className="text-center mt-10 opacity-70">
                  ...
                  <p className="text-sm text-muted-foreground">Ceritakan ide Anda untuk memulai.</p>
                </div>
              )}
              ...
              {messages.map((m) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex shrink-0 items-center justify-center ${m.role === 'user' ? 'bg-slate-800 text-white' : 'bg-yellow-400/20 text-yellow-400'}`}
                  >
                    {m.role === 'user' ? (
                  ...
                  <div
                    className={`rounded-2xl px-4 py-3 text-sm overflow-hidden ${m.role === 'user' ? 'bg-slate-800 text-white rounded-tr-sm' : 'bg-white/5 text-slate-200 rounded-tl-sm border border-white/5'}`}
                  >
                    <ReactMarkdown
                      components={{
                        code({ node, className, children, ...props }: any) {
                          ...
                          return (
                            <code
                              className="bg-black/30 px-1.5 py-0.5 rounded text-yellow-300 text-xs whitespace-pre-wrap break-all"
                              {...props}
                            >
                              {children}
                            </code>
