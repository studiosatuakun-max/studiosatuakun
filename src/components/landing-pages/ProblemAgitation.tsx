...
export function ProblemAgitation({ title, problems, theme = 'dark' }: ProblemAgitationProps) {
  ...
  return (
    <section
      className={`py-20 md:py-32 ${isLight ? 'bg-white text-slate-900' : 'bg-slate-950 text-slate-50'}`}
    >
      <div className="container mx-auto px-4 sm:px-8 max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {problems.map((problem, index) => (
            ...
            >
              <div
                className={`p-3 rounded-full ${isLight ? 'bg-red-100 text-red-500' : 'bg-red-500/10 text-red-400'}`}
              >
