          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {posts.map((post) => (
              <Link
                href={`/blog/${post.slug}`}
                key={post.slug}
                className="group flex flex-col bg-[#0c0c0c] border border-white/10 rounded-2xl overflow-hidden hover:border-yellow-500/50 transition-all duration-300 shadow-lg hover:shadow-yellow-500/10"
              >
                {post.coverImage ? (
                  ...
                  <div className="aspect-[16/9] w-full bg-gradient-to-br from-blue-900/40 to-purple-900/40 flex items-center justify-center border-b border-white/10">
                    <span className="font-bold text-white/30 text-2xl tracking-widest uppercase">
                      INSIGHT
                    </span>
                  </div>
                )}
