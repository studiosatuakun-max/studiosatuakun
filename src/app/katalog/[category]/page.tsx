    ...
    <main className="relative bg-paper text-ink min-h-screen flex flex-col">
      ...
      <div className="flex-1">
        <div className="px-6 md:px-10 pt-32 pb-16 border-b border-[rgba(107,110,115,0.25)]">
          <Link href="/#katalog" className="inline-flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-graphite hover:text-ink transition-colors mb-8">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <h1 className="font-mono text-4xl md:text-5xl font-light tracking-tight">{categoryInfo.title}</h1>
          </div>
        ...
        <div className="px-6 md:px-10 py-16">
          {templates.length === 0 ? (
            <div className="text-center py-20 border border-dashed border-[rgba(107,110,115,0.25)] flex flex-col items-center">
              ...
              <p className="font-mono text-sm text-graphite uppercase tracking-widest">No foundations yet</p>
              <p className="font-sans text-sm text-graphite mt-2">Check back soon for new additions to this category.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {templates.map(template => (
                <div key={template.id} className="group border border-[rgba(107,110,115,0.25)] hover:border-ink transition-colors duration-300 flex flex-col">
                  <Link href={template.previewUrl} className="block relative h-[250px] bg-[rgba(107,110,115,0.05)] border-b border-[rgba(107,110,115,0.25)] flex items-center justify-center overflow-hidden group-hover:bg-[rgba(107,110,115,0.08)] transition-colors">
                    {template.imageUrl ? (
                      <AppImage 
                      ...
                      <Icon name={categoryInfo.iconName} size={48} className="text-graphite opacity-20 relative z-10" />
                    )}
                  ...
                  <div className="p-8 flex flex-col flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 mb-8">
                      {template.techStack.map(tech => (
                        <span key={tech} className="font-mono text-[9px] uppercase tracking-widest px-2 py-1 bg-[rgba(107,110,115,0.05)] text-graphite">
                          {tech}
                        </span>
                      ))}
                    </div>
