...
const services: Service[] = [
  {
    ...
    detail: 'Bespoke web solutions tailored to your unique business needs, featuring cutting-edge technologies and fully customized architectures.',
  ...
  },
  {
    ...
    detail: 'Focus on conversion, SEO, high-speed access, and visual appeal with Bento Grid and smooth animations. Perfect for personal branding or product showcase.',
  ...
  },
  {
    ...
    detail: 'Highly interactive web applications with robust state management, authentication, and database logic. Ideal for marketplaces, SaaS, and community portals.',
  ...
  },
  {
    ...
    detail: 'Data visualization with charts, tables, complex CRUD operations, reporting, and role/permission management. For Logistics, HR, Finance.',
  ...
  },
  {
    ...
    detail: 'Custom solutions built from scratch, third-party API integrations, and modular architecture for unique business logic. For Enterprise clients.',
  ...
  },
  {
    ...
    title: 'AI Automation',
    price: 'Retainer',
    detail: 'Integrating LLMs, multimodal AI tools, and automated workflows to significantly increase client operational efficiency.',
          ...
          >
            <div className="px-6 md:px-10 py-6 flex items-center justify-between">
              <div className="flex items-baseline gap-6 md:gap-10">
                <span className={`font-mono text-xs opacity-40 ${open === svc.num ? 'text-paper' : 'text-ink'}`}>
                  {svc.num}
                </span>
                <h3 className={`font-mono text-xl md:text-3xl font-light tracking-tight ${open === svc.num ? 'text-paper' : 'text-ink'}`}>
                  {svc.title}
                </h3>
              </div>
              <div className="flex items-center gap-6">
                <span className={`font-mono text-xs uppercase tracking-widest hidden md:block ${open === svc.num ? 'text-paper/60' : 'text-graphite'}`}>
                  {svc.price}
                </span>
                <div className={`transition-transform duration-300 ${open === svc.num ? 'rotate-45' : ''}`}>
                  <Icon
                    name="PlusIcon"
                    size={16}
            ...
            <div className={`accordion-content ${open === svc.num ? 'open' : ''}`}>
              <div className="px-6 md:px-10 pb-8 md:pl-[calc(2.5rem+1.5rem+2.5rem)]">
                <p className={`font-sans text-sm leading-relaxed mb-4 max-w-2xl ${open === svc.num ? 'text-paper/70' : 'text-graphite'}`}>
                  {svc.detail}
                </p>
